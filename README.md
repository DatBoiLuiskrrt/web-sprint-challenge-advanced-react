...
- functional components are typically created using arrow functions, they accept data and display them. There is no render methods, no constructor and no render. Functional components take in props. Class components are classes that extend the component class in React. They tend to implement logic and state. React lifecycle methods can be used inside class components. You can pass props into class components using this.props.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

- A componentWillMount will be called after render. It is usually used to call asynchronous functions such as api calls.
- A componentDidUpdate method allows to execute the code when the component is updated. Network requests that are to be made when the props passed to the component changes are coded here. It is called after componentdidmount and is triggered everytime the state of the component changes
